...
For web and preview blocks, only `view` (and `url` or `file`) are needed. For terminal blocks that run a specific command, you must also set `"controller": "cmd"`:
...
Without `"controller": "cmd"`, a terminal block will open but remain blank — no command will run.

Use `"cmd:shell": true` when your command contains shell syntax like `&&`, pipes, or redirections. Use `"cmd:args"` for simple argument lists. Add `"cmd:interactive": true` for TUI apps (emacs, ncdu, superfile, etc.) that need an interactive terminal.
